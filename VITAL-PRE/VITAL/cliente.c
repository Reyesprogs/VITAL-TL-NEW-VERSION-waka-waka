...
#include "panel.h"
#include "admin.h" // <-- La nueva cabecera del Administrador
#include "inter.h"

// --- PANTALLA DE ERRORES DEL FORMULARIO ---
    ...
    while(1) {
        ...
        if (ch == KEY_UP) foco = (foco - 1 + 4) % 4; 
            ...
            if (foco == 3) return; 
                ...
                // --- VALIDACION DE NIVELES DE ACCESO ---
                if (strcmp(respuesta_servidor, "LOGIN_ADMIN") == 0) {
                    mostrar_notificacion("ACCESO AUTORIZADO. Bienvenido al Sistema Base.", 1);
                    abrir_panel_admin(semid, memoria); // Redirige al panel del Admin
                    return; 
                } 
                else if (strcmp(respuesta_servidor, "LOGIN_EXITO") == 0) {
                    mostrar_notificacion("Acceso CORRECTO. Bienvenido a VITAL-TL.", 1);
                    abrir_panel_principal(campos[0], semid, memoria); // Redirige al panel de Pacientes
                    return; 
                } else {
            ...
            if (foco < 2) { 
                int len = strlen(campos[foco]);
                if (len < 22) { campos[foco][len] = ch; campos[foco][len+1] = '\0'; }
            }
        }
    }
}
    ...
    while (1) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (i == 6) {
                ...
                for(int j = 0; j < longitud; j++) mvprintw(sy + 3 + (i * 2), sx + 16 + j, "*");
            } else {
        ...
        if (ch == KEY_UP) foco = (foco - 1 + 9) % 9; 
            ...
            if (foco == 8) return; 
                ...
                if (n_err > 0) {
                    ...
                    // Aseguramos el guardado con los nombres separados
                    snprintf(memoria->mensaje, 512, "Paterno: %s | Materno: %s | Nombre: %s | Sangre: %s | Correo: %s | Usr: %s | PassXOR: %s", 
